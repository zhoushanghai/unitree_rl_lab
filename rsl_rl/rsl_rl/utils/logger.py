...
class Logger:
    ...
    def log(
        ...
        ``extra_scalars``: optional full TensorBoard tag → value (e.g. ``{"Amp/mean_style_reward_step": 0.42}``).
        """
        if self.writer is not None:
            ...
            # 额外标量（如 AMP rollout 内 task / style 每步均值）；tag 需含分组前缀 ``Foo/bar``。
            if extra_scalars:
                for tag, value in extra_scalars.items():
                    self.writer.add_scalar(tag, value, it)  # type: ignore[arg-type]
            ...
            if extra_scalars:
                for tag, value in extra_scalars.items():
                    log_string += f"""{f"{tag}:":>{pad}} {value:.4f}\n"""
